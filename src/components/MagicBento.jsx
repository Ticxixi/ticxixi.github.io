  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      gridRef.current.querySelectorAll('.magic-bento-card').forEach(card => {
        ...
        const d = Math.hypot(e.clientX - (r.left + r.width / 2), e.clientY - (r.top + r.height / 2)) - Math.max(r.width, r.height) / 2;
        const ed = Math.max(0, d);
        minDistance = Math.min(minDistance, ed);
        const gi = ed <= proximity ? 1 : ed <= fadeDistance ? (fadeDistance - ed) / (fadeDistance - proximity) : 0;
        updateCardGlowProperties(card, e.clientX, e.clientY, gi, spotlightRadius);
      });
      gsap.to(spotlightRef.current, { left: e.clientX, top: e.clientY, duration: 0.1, ease: 'power2.out' });
      gsap.to(spotlightRef.current, { opacity: minDistance <= proximity ? 0.8 : minDistance <= fadeDistance ? ((fadeDistance - minDistance) / (fadeDistance - proximity)) * 0.8 : 0, duration: 0.4, ease: 'power2.out' });
    };
...
export default function MagicBento({
  ...
  enableTilt = false, glowColor = DEFAULT_GLOW_COLOR,
  ...
  const cards = [
    { color: '#141414', title: 'AI 内容创作', description: '策略→选题→拍摄→剪辑全流程', label: 'Content' },
    { color: '#141414', title: '大模型实践', description: 'GPT · Claude · Dola 深度使用', label: 'AI' },
    { color: '#141414', title: '行业调研', description: '一级市场信息 · 竞品分析 · 结构化简报', label: 'Research' },
    { color: '#141414', title: '产品设计', description: '需求分析到落地 · 跨平台开发', label: 'Product' },
    { color: '#141414', title: '中英双语', description: '英文文案 · 商务沟通 · 海外推广', label: 'Language' },
    { color: '#141414', title: '社交运营', description: '小红书1万+粉丝 · 单篇30万+浏览', label: 'Social' },
  ];
    ...
    <>
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cards.map((card, index) => {
          const baseCls = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''}`;
          const style = { backgroundColor: card.color, '--glow-color': glowColor };
          if (!enableStars) {
            return <div key={index} className={baseCls} style={style}>
          ...
          return (
            <ParticleCard key={index} className={baseCls} style={style} disableAnimations={shouldDisable} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
              <div className="magic-bento-card__header"><div className="magic-bento-card__label">{card.label}</div></div>
              <div className="magic-bento-card__content"><h2 className="magic-bento-card__title">{card.title}</h2><p className="magic-bento-card__description">{card.description}</p></div>
            </ParticleCard>
          );
